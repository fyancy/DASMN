...
class TCA:
    ...
    def fit_predict(self, Xs, Ys, Xt, Yt, split):
        """
        ...
        save_fig_path = r"C:\Users\Asus\Desktop\MyWork\paper_DASMN\DASMN_KBS_revised\figure\case_tsne\TCA\TCA_Csq_30.svg"
        plot_adaptation(Xs_new, Xt_new, shot=split)
        plt.show()
...
def main(way, split, dim):
    ...
    # --for DA tSNE---
    n_this = 30  # 50 for t-SNE; 10/30/50/100 for comparison
    split = 30
    ...
    acc, y_pre = model.fit_predict(Xs=train_x, Ys=train_y, Xt=test_x, Yt=test_y, split=split)
