...
def train_operate(way, ns, nq, save_path, final_test=True, load_path=None):
    ...
    #                             normalize=True, data_len=DIM, SNR=None, label=False, set='sa')
...
if __name__ == "__main__":
    ...
    n_cls = 10  # 10, 3
    ns = nq = 5
    m_f_root = r"G:\model_save\revised_DASMN\CW10\DASMN"

    # train:
    flag = input('Train? y/n\n')
    if flag == 'y' or flag == 'Y':
        # save_path = os.path.join(m_f_root, 'dasmn_cw3to0_' + str(running_params['train_split']))
        save_path = os.path.join(m_f_root, 'dasmn_funnySA_' + str(running_params['train_split']))
